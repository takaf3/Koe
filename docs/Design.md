...
Koe is a quiet native utility for switching between English, Japanese, and French dictation. The main surface is a 360-point menu bar popover. Recording gets a compact, nonactivating panel above the bottom edge of the active display.
...
```
 ...
 [ Auto | Japanese | English | French ]
 English, Japanese, and French, chosen per recording.
...
The native grouped settings support this brief better than a separate app window or a decorative dashboard. Text stays left aligned; settings and status values align on the right. An ambiguous automatic result offers all usable transcripts in a scrollable chooser before insertion.
