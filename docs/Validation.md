...
- All 47 XCTest cases passed, including automatic model download for added languages (queued while another download runs), legacy `japanese`/`english`/`french` preference migration, fresh-install defaults derived from the system’s preferred languages, adding and removing languages with mode fallback, restoring a saved fixed language that was not in the enabled list, the Auto-detect warning past three languages, region-aware names for two variants of one language, readiness messages for unavailable and downloading models, and the widened uncertainty margin.
